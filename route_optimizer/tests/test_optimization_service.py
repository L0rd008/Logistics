...
class TestOptimizationService(unittest.TestCase):
    ...
    def test_add_detailed_paths(self):
        """Test adding detailed path information to the optimization result."""
        # Sample result without detailed paths
        result = {
            'status': 'success',
            'routes': [[0, 1, 2, 0], [0, 3, 0]],
            'total_distance': 6.0
        ...
        # Mock the path finder
        self.service.path_finder.calculate_shortest_path = MagicMock(return_value=(['a', 'b'], 1.0))
        
        # Call the method
        self.service._add_detailed_paths(result, graph)
        
        # Verify detailed paths were added
        self.assertIn('detailed_paths', result)
        self.service.path_finder.calculate_shortest_path.assert_called()

    def test_add_summary_statistics(self):
        """Test adding summary statistics to the optimization result."""
        # Sample result without statistics
        result = {
            'status': 'success',
            'routes': [[0, 1, 2, 0], [0, 3, 0]],
            'total_distance': 6.0,
        ...
        self.assertIn('num_vehicles_used', result['statistics'])
        self.assertEqual(result['statistics']['num_vehicles_used'], 2)
        
        # Other statistics should be present
        self.assertIn('total_capacity', result['statistics'])
        self.assertIn('utilization_percentage', result['statistics'])
